...
- **The map's route *line*** is road-snapped via a one-time Directions API call per route load (`src/routing/directions.ts`), purely for display — falls back to straight lines between waypoints if that call fails. This is separate from and doesn't affect turn-by-turn guidance above, which always uses the raw waypoints. Needs its own API key (`src/config/secrets.ts`, gitignored — copy from `secrets.example.ts`) because the Android-app-restricted Maps key doesn't work for a plain REST call; this one is restricted to the Directions API only, no application restriction.
